...
public class TextObject extends GameObject {
   ...
   * @param playground to be drawn on
   * @param controller which takes over the movement or the disappearance
   * @param x position
   * @param y position
   * @param vx velocity (speed)
   * @param vy velocity (speed)
   * @param text which is displayed on the playground
   * @param size of the text
   */
  public TextObject(String id, Playground playground, ObjectController controller, double x,
      double y, double vx, double vy, String text, int size) {
  ...
  public void draw(Graphics2D g) {
    ...
    g.drawString(as.getIterator(), (int) Math.round(this.x - this.rx),
        (int) Math.round(this.y - this.ry));
  }
}
